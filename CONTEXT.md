...
_Avoid_: Partner listing, Promoted service, Affiliate (separate types — never call them Official)

**Partner listing**:
Curated outbound hop to a **non-gov** org we willingly list — **never** labeled Official. Decision: [ADR-0013](docs/adr/0013-partner-listing.md). **Allowlist v1:** membership-based industry / trade / civic associations (e.g. BASIS, FBCCI, BGMEA-class) with a public useful portal. **Not published yet** (no Partner hops, schema, or Instant Directory inclusion until a ship wave + discovery ADR). **Out of Partner for this slice:** private banks, MFS apps, commercial SaaS; Promoted and Affiliate remain separate future types.
_Avoid_: Calling Partner listings Official; stuffing associations into Official Services; shipping Partner content before the ship wave

**Out of catalog (v1)**:
Private banks, MFS apps (bKash/Nagad as products), commercial SaaS, and purely promotional third-party tools. Half-gov / SOE / state banks are **not** out of catalog. Industry / trade associations are **not Official** and are **not published** as Partner hops yet (ADR-0013 locks the type only).
_Avoid_: Calling these Official Services; treating association portals as Official candidates
