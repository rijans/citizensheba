---
id: moedu
slug: bd-moedu
title: Ministry of Education
title_bn: শিক্ষা মন্ত্রণালয়
description: Official Ministry of Education portal for national education policy notices and ministry information.
description_bn: জাতীয় শিক্ষা নীতি নোটিশ ও মন্ত্রণালয় তথ্যের অফিসিয়াল শিক্ষা মন্ত্রণালয় পোর্টাল
body: |
  **Ministry of Education** is the official portal for national education ministry notices, policy information, and published guidance.

  Board results and many student hops remain on dedicated education Service Pages (results, admission, NCTB, DSHE).

body_bn: |
  **শিক্ষা মন্ত্রণালয়** জাতীয় শিক্ষা মন্ত্রণালয়ের নোটিশ, নীতি তথ্য ও প্রকাশিত নির্দেশনার অফিসিয়াল পোর্টাল।

  বোর্ড ফলাফল ও অনেক শিক্ষার্থী হপ নিবেদিত শিক্ষা সেবা পেজে (ফলাফল, ভর্তি, এনসিটিবি, মাউশি)।

url: "https://www.moedu.gov.bd/"
official_domain: moedu.gov.bd
category: education
directory_global_rank: 890
directory_category_rank: 40
tags: [education ministry, moedu, "শিক্ষা মন্ত্রণালয়"]
aliases:
  - name: MoE Bangladesh
    lang: en
    kind: alt
  - name: Education Ministry
    lang: en
    kind: informal
  - name: শিক্ষা মন্ত্রণালয়
    lang: bn
    kind: alt
  - name: মন্ত্রণালয় শিক্ষা
    lang: bn
    kind: informal
status: ACTIVE
audience: Students, teachers, and schools following official Ministry of Education information.
audience_bn: অফিসিয়াল শিক্ষা মন্ত্রণালয় তথ্য অনুসরণকারী শিক্ষার্থী, শিক্ষক ও স্কুল।
faq:
  - q: "Where do I check board exam results?"
    a: "Use the Education Board Results portal. The ministry portal publishes policy and notices."
    q_bn: "বোর্ড পরীক্ষার ফলাফল কোথায় দেখব?"
    a_bn: "শিক্ষা বোর্ড ফলাফল পোর্টাল ব্যবহার করুন। মন্ত্রণালয় পোর্টাল নীতি ও নোটিশ প্রকাশ করে।"
related: [edu-results, dshe, nctb, mopme, tmed]
last_verified: 2026-08-07
---

