---
id: bangladesh-post
slug: bd-bangladesh-post
title: Bangladesh Post
title_bn: বাংলাদেশ পোস্ট
description: Official Bangladesh Post portal for postal services, tracking information, and published post-office guidance.
description_bn: ডাক সেবা, ট্র্যাকিং তথ্য ও প্রকাশিত পোস্ট অফিস নির্দেশনার অফিসিয়াল বাংলাদেশ পোস্ট পোর্টাল
body: |
  **Bangladesh Post** is the official portal for Bangladesh postal services, tracking information, and post-office related guidance.

  Use it for official postal notices and service entry points published by Bangladesh Post.

body_bn: |
  **বাংলাদেশ পোস্ট** বাংলাদেশের ডাক সেবা, ট্র্যাকিং তথ্য ও পোস্ট অফিস-সংক্রান্ত নির্দেশনার অফিসিয়াল পোর্টাল।

  বাংলাদেশ পোস্ট প্রকাশিত অফিসিয়াল নোটিশ ও সেবা এন্ট্রির জন্য এটি ব্যবহার করুন।

url: "https://www.bangladeshpost.gov.bd/"
official_domain: bangladeshpost.gov.bd
category: central
tags: [post, postal, tracking, "ডাক", "পোস্ট"]
aliases:
  - name: Bangladesh Postal Department
    lang: en
    kind: alt
  - name: postal tracking
    lang: en
    kind: informal
  - name: বাংলাদেশ ডাক
    lang: bn
    kind: alt
  - name: পোস্ট অফিস
    lang: bn
    kind: informal
status: ACTIVE
audience: People using Bangladesh Post for mail, parcels, and official postal information.
audience_bn: চিঠি, পার্সেল ও অফিসিয়াল ডাক তথ্যের জন্য বাংলাদেশ পোস্ট ব্যবহারকারী নাগরিক।
faq:
  - q: "Is courier private delivery the same as Bangladesh Post?"
    a: "No. Private couriers are separate. This Service Page points to the official Bangladesh Post portal."
    q_bn: "প্রাইভেট কুরিয়ার কি বাংলাদেশ পোস্টের মতোই?"
    a_bn: "না। প্রাইভেট কুরিয়ার আলাদা। এই সেবা পেজ অফিসিয়াল বাংলাদেশ পোস্ট পোর্টালে যায়।"
related: [national-portal, mygov]
last_verified: 2026-08-07
---
