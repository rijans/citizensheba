---
id: biwta
slug: bd-biwta-inland-water-transport-authority
title: BIWTA
title_bn: বিআইডব্লিউটিএ
serp_title: BIWTA (Inland Water Transport Authority)
serp_title_bn: বিআইডব্লিউটিএ (অভ্যন্তরীণ নৌপরিবহন কর্তৃপক্ষ)
description: Official Bangladesh Inland Water Transport Authority portal for waterway regulation and navigation information.
description_bn: নৌপথ নিয়ন্ত্রণ ও নৌচলাচল তথ্যের অফিসিয়াল বাংলাদেশ অভ্যন্তরীণ নৌপরিবহন কর্তৃপক্ষ পোর্টাল
body: |
  **BIWTA** (Bangladesh Inland Water Transport Authority) is the official portal for inland waterway regulation, port/terminal-related notices, and navigation information.

  Passenger launch operations information may also appear on BIWTC; use BIWTA for authority-level waterway guidance.

body_bn: |
  **বিআইডব্লিউটিএ (BIWTA)** অভ্যন্তরীণ নৌপথ নিয়ন্ত্রণ, পোর্ট/টার্মিনাল-সংক্রান্ত নোটিশ ও নৌচলাচল তথ্যের অফিসিয়াল পোর্টাল।

  যাত্রীবাহী লঞ্চ সেবা তথ্য বিআইডব্লিউটিসিতেও থাকতে পারে; কর্তৃপক্ষ-পর্যায়ের নৌপথ নির্দেশনার জন্য বিআইডব্লিউটিএ ব্যবহার করুন।

url: "https://biwta.gov.bd/"
official_domain: biwta.gov.bd
category: transport
directory_global_rank: 640
directory_category_rank: 70
icon: anchor
capabilities:
  - en: "View waterway information"
    bn: "নৌপথ তথ্য দেখুন"
  - en: "Read navigation notices"
    bn: "নৌচলাচল নোটিশ পড়ুন"
  - en: "Find regulation updates"
    bn: "নিয়ন্ত্রণ আপডেট খুঁজুন"
tags: [biwta, waterway, navigation, "বিআইডব্লিউটিএ", "নৌপথ"]
aliases:
  - name: Bangladesh Inland Water Transport Authority
    lang: en
    kind: alt
  - name: inland water authority
    lang: en
    kind: informal
  - name: অভ্যন্তরীণ নৌপরিবহন কর্তৃপক্ষ
    lang: bn
    kind: alt
  - name: নৌ কর্তৃপক্ষ
    lang: bn
    kind: informal
status: ACTIVE
audience: Travellers and operators following official BIWTA inland waterway authority information.
audience_bn: অফিসিয়াল বিআইডব্লিউটিএ অভ্যন্তরীণ নৌপথ কর্তৃপক্ষ তথ্য অনুসরণকারী যাত্রী ও পরিচালক।
faq:
  - q: "Difference between BIWTA and BIWTC?"
    a: "BIWTA is the inland water transport authority. BIWTC is the corporation that operates many water transport services. Check each portal for its published role."
    q_bn: "বিআইডব্লিউটিএ ও বিআইডব্লিউটিসির পার্থক্য?"
    a_bn: "বিআইডব্লিউটিএ অভ্যন্তরীণ নৌপরিবহন কর্তৃপক্ষ। বিআইডব্লিউটিসি অনেক নৌপরিবহন সেবা পরিচালনাকারী কর্পোরেশন। প্রকাশিত ভূমিকা প্রতিটি পোর্টালে দেখুন।"
related: [biwtc, railway, brta, caab, cpa, mpa, mos, bsc]
last_verified: 2026-08-07
---

