...
- **Service hop content regions:** About = primary surface panel; Who/FAQ = muted support strips; all three use chevron disclose (**open by default**); labeled BN/EN panes — see [`service-page.md`](service-page.md). `.lang-label` stays the same small size in every region (exclude it from support/FAQ `p` font-size rules — Trap #15).
